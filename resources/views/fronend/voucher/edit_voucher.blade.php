    ...
    <div class="col-lg-12 mb20">
        <div class="ibox float-e-margins">
            ...
            <div class="ibox-content">
                <form id="product-form" action="{{ route('voucher_update', $voucher->id) }}" method="POST"
                    ...
                    <div class="content-left">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="name" class="form-control"
                                value="{{ old('name', $voucher->name) }}" placeholder="Nhập tên chương trình...">
                        </div>
                        @error('name')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                        <div class="form-group">
                            ...
                            <select name="type" class="form-control" id="discount-type">
                                <option value="percent"
                                    {{ old('type', $voucher->type ?? '') == 'percent' ? 'selected' : '' }}>Giảm
                                    theo phần trăm
                                </option>
                    ...
                    <div class="content-right">
                        <div class="form-group">
                            ...
                            <input type="datetime-local" name="start_date" id="start_date" class="form-control"
                                ...
                                value="{{ old('start_date', isset($voucher) && $voucher->start_date ? \Carbon\Carbon::parse($voucher->start_date)->format('Y-m-d\TH:i') : '') }}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="datetime-local" name="end_date" class="form-control" max="9999-12-31T23:59"
                                value="{{ old('end_date', isset($voucher) && $voucher->end_date ? \Carbon\Carbon::parse($voucher->end_date)->format('Y-m-d\TH:i') : '') }}">
                        </div>
                        @error('end_date')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control">
                                <option value="1" {{ old('status', $voucher->status ?? 1) == 1 ? 'selected' : '' }}>
                                    Kích hoạt</option>
                                <option value="0" {{ old('status', $voucher->status ?? 1) == 0 ? 'selected' : '' }}>
                                    Vô
                                    hiệu hóa</option>
                            </select>
                        </div>
                    ...
                    <div class="form-actions">
                        ...
                        <button type="submit" class="btn-primary">Cập nhật voucher</button>
                    </div>
                </form>
            </div>
